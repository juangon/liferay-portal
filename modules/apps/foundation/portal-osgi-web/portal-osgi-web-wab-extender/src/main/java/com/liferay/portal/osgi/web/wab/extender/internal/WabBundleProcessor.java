...
public class WabBundleProcessor {
	...
	public void init(
		...
		try {
			...
			List<WebXMLDefinition> sortedWebXML = new ArrayList<>();

			if (ListUtil.isNotEmpty(webFragments)) {
				sortedWebXML = WebFragmentOrderUtil.getOrder(
					webFragments, webXMLDefinition.getAbsoluteOrderNames());
			}

			webXMLDefinition = assembleWebXML(webXMLDefinition, sortedWebXML);
