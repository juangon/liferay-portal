...
public class JGroupsCachePeer implements CachePeer {
	...
	private static final Log LOG = LogFactoryUtil.getLog(JGroupsCachePeer.class);
    ...
    public void send(Address dest, List<JGroupEventMessage> eventMessages) {
        if (!this.alive || eventMessages == null || eventMessages.isEmpty()) {
            LOG.warn("Ignoring send request of " + eventMessages == null ? null : eventMessages.size() + " messages. Replicator alive = " + this.alive);
            return;
        }
        ...
        for (final JGroupEventMessage groupEventMessage : eventMessages) {
            if (groupEventMessage.isAsync()) {
                ...
                LOG.trace("Queued " + groupEventMessage + " for asynchronous sending.");
            } else {
                synchronousEventMessages.add(groupEventMessage);
                LOG.trace("Sending " + groupEventMessage + " synchronously.");
            }
        }
        ...
        LOG.debug("Sending " + synchronousEventMessages.size() + " JGroupEventMessages synchronously.");
    ...
    private Queue<JGroupEventMessage> getMessageQueue(long asyncTime) {
        ...
        if (queue == null) {
            ...
            if (queue == null) {
                LOG.debug("Created asynchronous message queue for " + asyncTime + "ms period");
    ...
    private void flushQueue(Queue<JGroupEventMessage> queue) {
        ...
        while (!queue.isEmpty()) {
            ...
            LOG.debug("Sending " + events.size() +" JGroupEventMessages from the asynchronous queue.");
