...
public class JGroupsBootstrapManager {
    private static final Log LOG = LogFactoryUtil.getLog(JGroupsBootstrapCacheLoaderFactory.class);
    ...
    public void dispose() {
        ...
        if (!this.bootstrapRequests.isEmpty()) {
            ...
            if (!this.bootstrapRequests.isEmpty()) {
                LOG.warn("Shutting down bootstrap manager while there are still " +  this.bootstrapRequests.size() + " bootstrap requests pending");
            }
        }
        ...
        try {
            if (!this.bootstrapThreadPool.awaitTermination(BOOTSTRAP_RESPONSE_TIMEOUT, TimeUnit.MILLISECONDS)) {
                LOG.warn("Not all bootstrap threads shutdown within " + BOOTSTRAP_RESPONSE_TIMEOUT + "ms window");
            }
    ...
    public void handleBootstrapRequest(BootstrapRequest bootstrapRequest) {
        ...
        if (!this.referenceTimerScheduled.getAndSet(true)) {
            ...
            LOG.debug("Scheduled BootstrapRequest Reference cleanup timer with " + BOOTSTRAP_REQUEST_CLEANUP_INTERVAL + "ms period");
        }
        ...
        if (oldRequest != null) {
            LOG.warn("There is already a BootstrapRequest registered for " + cacheName + "with value " + oldRequest + ", it has been replaced with the current request.");
        }
        ...
        if (!bootstrapRequest.isAsynchronous()) {
            LOG.debug("Waiting up to " + BOOTSTRAP_RESPONSE_MAX_TIMEOUT + "ms for BootstrapRequest of " + cacheName + " to complete");
            try {
    ...
    public void handleBootstrapComplete(JGroupEventMessage message) {
        ...
        if (bootstrapRequestStatus != null) {
            ...
            LOG.warn("No BootstrapRequest registered for cache " + cacheName + ", the event will have no effect: "  + message);
        }
    }
        ...
        public void runInternal() {
            ...
            try {
                ...
                if (addresses == null || addresses.size() == 0) {
                    LOG.info("There are no other nodes in the cluster to bootstrap " + cacheName + "from");
                    return;
                }
                
                final Address localAddress = cachePeer.getLocalAddress();
                LOG.debug("Loading cache "  + cacheName + " with local address " + localAddress + " from peers: " + addresses);
                ...
                do {
                    ...
                    if (LOG.isDebugEnabled()) {
                        LOG.debug("Requesting bootstrap of " + cacheName + " from " + address);
                    }
                ...
                if (BootstrapStatus.COMPLETE == bootstrapRequest.getBootstrapStatus()) {
                    LOG.info("Bootstrap for cache " + cacheName + " is complete, loaded " + replicationCount + " elements");
                } else {
                    LOG.info("Bootstrap for cache " + cacheName + " ended with status " + bootstrapRequest.getBootstrapStatus() +", loaded " + replicationCount + " elements");
                }
            } finally {
                final BootstrapRequest removedRequest = bootstrapRequests.remove(cacheName);
                if (removedRequest == null) {
                    LOG.warn("No BootstrapRequest for " + cacheName + " to remove");
                    return;
                }
        ...
        protected void waitForBootstrap(final String cacheName, final Address address) {
            ...
            for (int waitTry = 1; waitTry <= BOOTSTRAP_RESPONSE_TRIES; waitTry++) {
                try {
                    ...
                    LOG.debug("Bootstrap of " + cacheName + " did not complete in " + (BOOTSTRAP_RESPONSE_TIMEOUT * waitTry) + "ms, will wait " + (BOOTSTRAP_RESPONSE_TIMEOUT - waitTry) + " more times.");
                } catch (InterruptedException e) {
                    LOG.warn("Interrupted while waiting for bootstrap of " + cacheName + " to complete", e);
                }
            }
            
            LOG.warn("Bootstrap of " + cacheName + " did not complete in " + BOOTSTRAP_RESPONSE_MAX_TIMEOUT + "ms, giving up on bootstrap request to " + address + ".");
        }
    ...
    private final class BootstrapResponseRunnable extends ThreadNamingRunnable {
        ...
        public void runInternal() {
            ...
            if (cache == null) {
                LOG.warn("ignoring bootstrap request:   from " + requestAddress + " for cache " + cacheName + " which does not exist on this memeber");
            ...
            LOG.debug("servicing bootstrap request: from " + requestAddress + " for cache=" +  cacheName);
            ...
            if (bootstrapRequests.get(cacheName) != null) {
                LOG.debug("This group member is currently bootstrapping " + cacheName + " from another node and cannot respond to a " +
                        "bootstrap request for this cache. Notifying requester of incomplete bootstrap");
            ...
            if (keys == null || keys.size() == 0) {
                LOG.debug("no keys to reply to " + requestAddress + " to bootstrap cache " + cacheName);
            } else {
        ...
        private void sendResponseChunk(Ehcache cache, Address requestAddress, List<JGroupEventMessage> events) {
            LOG.debug("reply " + events.size() + " elements to " + requestAddress + " to bootstrap cache " + cache.getName());
