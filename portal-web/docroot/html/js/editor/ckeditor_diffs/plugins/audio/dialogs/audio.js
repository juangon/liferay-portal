		...
		return {
			...
			onShow: function() {
				...
				if (fakeImage && fakeImage.data('cke-real-element-type') && fakeImage.data('cke-real-element-type') === 'audio') {
					...
					instance.audioNode = audioNode;

					instance.setupContent(audioNode);
				}
				else {
			...
			onOk: function() {
				...
				divNode.setAttribute('class', 'liferayckeaudio audio-container');

				var boundingBoxTmp = editor.document.createElement(STR_DIV);

				boundingBoxTmp.setAttribute('class', 'ckaudio-no-id');
